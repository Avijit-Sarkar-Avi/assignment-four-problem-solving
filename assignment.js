...
function radianToDegree(radian) {
    if (typeof (radian) === "number") {
        const degree = (parseFloat(((180 * radian) / Math.PI).toFixed(2)));
        return degree;
    }
    else {
...
let myValue = 199;
let theOutput = radianToDegree(myValue);
...
let theString = 'index.png.js';
let theOut = isJavaScriptFile(theString);
// console.log(theOut);
...
function oilPrice(price1, price2, price3) {
    const diselPrice = 114;
    const pactrolPrice = 130;
    const octanPrice = 135;
    let totalCost = ((price1 * diselPrice) + (price2 * pactrolPrice) + (price3 * octanPrice));
    return totalCost;
}
...
console.log(oilPrice(disel, pactrol, octan));
...
function publicBusFare(totalPerson) {
    let busPerson = totalPerson % 50;
    let microbusPerson = busPerson % 11;
    const publicbusPrice = 250;
    if (busPerson == 0) {
        // console.log('Go to the bus');
    }
    else if (busPerson > 0 && microbusPerson == 0) {
        // console.log('Go to the microbus');
    }
    else if (busPerson > 0 && microbusPerson > 0) {
        let totalPublicbusCost = microbusPerson * publicbusPrice;
...
let frndOutput = isBestFriend(object1, object2);
console.log(frndOutput);
